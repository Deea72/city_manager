...
void add(char *district, char *nume_user)  //append a new report
{
    ...
    else   //if the directory does not exist we make it 
    {
    ...
    strcpy(reports_path, district);
    strcat(reports_path, "/reports.dat");


    int f = open(reports_path, O_WRONLY | O_APPEND | O_CREAT, 0664);   
    if (f == -1) {
        perror("Error opening reports.dat");
        return;
    }

    write(f, &nou, sizeof(REPORT));
    ...
    if(strcmp(argv[0], "./city_manager") == 0)  //suntem intr o comanda
    {
        ...
        if(argv[5])
        {
            ...
            {   
                ...
                {
                    if(role == 2)
                        update_threshold(argv[6],argv[7]);   
                    else
